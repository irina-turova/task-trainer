...
object DifficultyController {
    fun get(subtheme: String?): List<Difficulty> {
        val difficulties = ObjectSelect.query(Difficulty::class.java).where(Difficulty.TASKS.dot(Task.SUBTHEME1).dot(Subtheme.NAME).eq(subtheme)).select(OrmManager.context)
        return difficulties
    }
